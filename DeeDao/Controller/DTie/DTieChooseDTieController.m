...
@interface DTieChooseDTieController ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
- (void)createTopView
{
    ...
    titleLabel.text = DDLocalizedString(@"Add D Page");
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChooseDtie:)]) {
        [self.delegate didChooseDtie:self.chooseSource];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
...
{
    if (!_chooseSource) {
        _chooseSource = [[NSMutableArray alloc] init];
    }
