...
@interface ChooseUserViewController ()<UITableViewDelegate, UITableViewDataSource>
...
- (instancetype)initWithUsers:(NSMutableArray *)users
{
    if (self = [super init]) {
        ...
        self.dataSource = [[NSMutableArray alloc] initWithArray:users];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.selectFriend containsObject:model]) {
        [self.selectFriend removeObject:model];
...
- (void)createTopView
{
    ...
    titleLabel.text = DDLocalizedString(@"Author");
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(userDidCompleteSelectWith:)]) {
        [self.delegate userDidCompleteSelectWith:self.selectFriend];
