...
- (void)createCollectionTableCell
{
    ...
    [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.timeLabel.mas_bottom).offset(20 * scale);
        make.left.mas_equalTo(60 * scale);
...
- (void)configWithModel:(DTieEditModel *)model
{
    if (model.type == DTieEditType_Image) {
        ...
        self.readLabel.text = model.detailContent;
    }
}

- (void)configWithModel:(DTieEditModel *)model firstTime:(NSString *)firstTime location:(NSString *)location updateTime:(NSString *)updateTime
{
    if (model.type == DTieEditType_Image) {
        self.firstReadView.hidden = YES;
        self.baseReadView.hidden = YES;
        ...
        self.baseReadView.hidden = YES;
        self.baseImageView.hidden = YES;
        self.firstReadLabel.text = model.detailContent;
