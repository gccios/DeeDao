...
- (void)createMineHeaderView
{
    ...
    [self.headerImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.mas_equalTo(0);
        make.top.mas_equalTo(225 * scale);
    }];
    self.headerImageView.layer.cornerRadius = 110 * scale;
    ...
    self.nameLabel = [DDViewFactoryTool createLabelWithFrame:CGRectZero font:kPingFangRegular(54 * scale) textColor:UIColorFromRGB(0xDB6283) alignment:NSTextAlignmentLeft];
    self.nameLabel.text = [UserManager shareManager].user.nickname;
    [self addSubview:self.nameLabel];
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.headerImageView.mas_bottom).offset(40 * scale);
        make.centerX.mas_equalTo(self.headerImageView);
